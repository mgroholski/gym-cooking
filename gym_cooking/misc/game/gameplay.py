...
class GamePlay(Game):
    def __init__(self, filename, world, sim_agents):
        ...
        self.gridsquares = []
        self.gridsquare_types = defaultdict(
            set
        )  # {type: set of coordinates of that type}
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_RETURN:
                image_name = "{}_{}.png".format(
                    self.filename, datetime.now().strftime("%m-%d-%y_%H-%M-%S")
                )
                pygame.image.save(
                    self.screen, "{}/{}".format(self.save_dir, image_name)
                )
            ...
            if pygame.key.name(event.key) in "1234":
                try:
                    self.current_agent = self.sim_agents[
                        int(pygame.key.name(event.key)) - 1
                    ]
                except:
                    pass
                return
