...
from utils.agent import COLORS, RealAgent, SimAgent
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Save observation at each time step as an image in misc/game/record",
    )
...
def main_loop(arglist):
    ...
    bag.set_termination(
        termination_info=env.termination_info, successful=env.successful
    )
...
if __name__ == "__main__":
    ...
    if arglist.play:
        ...
        assert (
            len(list(filter(lambda x: x is not None, model_types)))
            == arglist.num_agents
        ), "num_agents should match the number of models specified"
